...
export const FocusPage: React.FC = () => {
  ...
  const MatrixSection = ({ title, type, color, icon: Icon, desc }: any) => {
      const items = state.todos.filter(t => t.matrix === type);
      return (
          <div className={`relative group overflow-hidden rounded-[32px] border transition-all duration-500 flex flex-col h-full bg-[#0f172a]/60 backdrop-blur-xl ${type === matrixType ? `border-${color}-500/50 shadow-[0_0_30px_rgba(0,0,0,0.3)]` : `border-${color}-500/20 hover:border-${color}-500/30`}`}>
              ...
              <div className="p-5 border-b border-white/5 flex items-center gap-3 relative z-10">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center bg-${color}-500/10 text-${color}-400 border border-${color}-500/20 shadow-lg`}>
                      <Icon size={18} />
                  </div>
                  <div>
                      <h3 className={`font-black text-${color}-400 text-xs uppercase tracking-[0.2em]`}>{title}</h3>
              ...
              <div className="flex-1 space-y-3 overflow-y-auto p-4 no-scrollbar relative z-10">
                  {items.length === 0 && (
                      <div className="h-full flex flex-col items-center justify-center opacity-30">
                          <Icon size={40} className={`text-${color}-500 mb-2`} />
                          <span className="text-xs font-bold uppercase tracking-wider">No Tasks</span>
                      </div>
                  )}
                  {items.map(t => (
                      <div key={t.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all duration-300 group/item ${t.completed ? 'bg-slate-900/50 border-transparent opacity-60' : `bg-slate-800/40 border-white/5 hover:bg-${color}-500/10 hover:border-${color}-500/20`}`}>
                          <button onClick={() => toggleTodo(t.id)} className={`transition-colors ${t.completed ? 'text-slate-500' : `text-${color}-400 hover:text-white`}`}>
            ...
            <div className="animate-slideUp">
                ...
                <div className="mb-8 bg-[#0f172a]/80 backdrop-blur-xl border border-white/10 rounded-[32px] p-2 shadow-xl flex flex-col md:flex-row gap-2 relative z-20">
                    <div className="flex p-1 gap-1 bg-slate-900/80 rounded-[24px] border border-white/5 overflow-x-auto no-scrollbar">
                        {([
                            { id: 'do', label: 'Do', color: 'emerald' },
                            { id: 'decide', label: 'Plan', color: 'blue' },
                            { id: 'delegate', label: 'Msg', color: 'amber' },
                            { id: 'delete', label: 'Kill', color: 'rose' }
                        ] as const).map((t) => (
                            <button 
                                key={t.id}
                                type="button" 
                                onClick={() => setMatrixType(t.id)}
                                className={`px-5 py-3 rounded-[20px] text-xs font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${matrixType === t.id ? `bg-${t.color}-500 text-white shadow-lg shadow-${t.color}-900/50` : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}
                            >
                                {t.label}
                            </button>
                        ))}
