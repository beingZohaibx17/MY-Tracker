...
export const RamadanPage: React.FC = () => {
  ...
  const TaskCard = ({ id, title, sub, icon, active, color }: any) => (
    <button 
      onClick={() => toggle(id)}
      className={`group relative p-6 rounded-[32px] border transition-all duration-500 flex flex-col items-center justify-center gap-4 overflow-hidden ${active ? `bg-${color}-900/20 border-${color}-500/50 shadow-[0_0_30px_rgba(0,0,0,0.2)]` : 'bg-slate-900/50 border-white/5 hover:bg-slate-800'}`}
    >
      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-500 ${active ? `bg-${color}-500 text-white shadow-[0_0_30px_rgba(255,255,255,0.2)] scale-110 rotate-3` : `bg-slate-800 text-slate-500 group-hover:scale-105`}`}>
        {icon}
      </div>
      ...
      {/* Glow effect */}
      {active && <div className={`absolute inset-0 bg-${color}-500/10 blur-xl`}></div>}
