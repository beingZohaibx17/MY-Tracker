...
export const HygienePage: React.FC = () => {
  ...
  const DetailCard = ({ title, icon: Icon, currentVal, currentLabel, overallVal, overallLabel, action, actionLabel, active, colorClass, limit }: any) => {
      ...
      return (
          <div className={`relative p-6 rounded-[32px] border transition-all duration-500 overflow-hidden group ${active ? `bg-[#0f172a] border-${colorClass}-500/50 shadow-[0_0_30px_rgba(0,0,0,0.2)]` : 'bg-slate-900/40 border-white/5 hover:bg-slate-900/60'}`}>
               <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-5 mix-blend-overlay pointer-events-none"></div>
               
               <div className="flex justify-between items-start mb-6 relative z-10">
                   <div className="flex items-center gap-4">
                       <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl transition-all duration-500 shadow-lg ${active ? `bg-${colorClass}-500 text-white scale-110 rotate-3` : `bg-slate-800 text-${colorClass}-400`}`}>
                           <Icon size={24} />
                       </div>
                       <div>
               ...
               <div className="flex items-end justify-between gap-4 relative z-10">
                   <div>
                        ...
                        <div className={`font-mono font-black text-4xl ${isLimitExceeded ? 'text-rose-500' : `text-${colorClass}-400`}`}>
                   ...
                   <button 
                    onClick={action}
                    className={`px-5 py-3 rounded-xl font-bold text-xs uppercase tracking-wide transition-all active:scale-95 shadow-lg ${active ? `bg-${colorClass}-500 text-white hover:bg-${colorClass}-400` : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                   >
                       {actionLabel}
                   </button>
               </div>
               ...
               {limit && (
                   <div className="mt-5 h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                       <div 
                        className={`h-full transition-all duration-500 ${isLimitExceeded ? 'bg-rose-500' : `bg-${colorClass}-500`}`}
